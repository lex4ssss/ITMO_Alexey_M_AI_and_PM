...
## Интеграции и точки вызова внешних сервисов

| Система | Где вызывается | Синхронность | Что при отказе |
|---|---|---|---|
| Каналы обращений (чат, email, веб-форма, мобильное приложение) | Ingest API, приём входящего | синхронно | обращение принимается каналом и ретраится; потеря недопустима |
| CRM / helpdesk | постановка тикета в очередь оператора, обновление статуса, чтение факта reopen | синхронно на записи маршрута, асинхронно на обогащении | маршрут пишется в собственную БД, синхронизация с CRM догоняет очередью |
| Профиль пользователя | обогащение тикета: тариф, история заказов, признак ценного клиента | асинхронно, до генерации | генерация идёт без обогащения, приоритет ставится по умолчанию |
| LLM-провайдер | Answer Generator, только на медленном пути | асинхронно, таймаут 10 с | генерация отключается, тикет уходит оператору (см. запасные пути) |
| Хранилище базы знаний | Retriever, инкрементальная переиндексация по расписанию | асинхронно | используется предыдущий индекс, алерт на устаревание |
| Система уведомлений | отправка ответа или подтверждения приёма пользователю | асинхронно | ретраи с экспоненциальной задержкой, при исчерпании — тикет оператору |
| Страница статуса сервиса | режим инцидента, ссылка в подтверждении приёма | асинхронно | подтверждение уходит без ссылки |
| Хранилище секретов | ключи LLM-провайдера и доступы к CRM | при старте сервиса | сервис не поднимается, работает предыдущая версия |
| Метрики и логи (Prometheus, Loki) | все компоненты | асинхронно, без блокировки | наблюдаемость теряется, обработка продолжается |

Общее правило: **ни один внешний вызов не находится на синхронном пути маршрутизации.**
В бюджет 500 мс входят только собственные вычисления и запись в свою БД, поэтому недоступность
любой внешней системы не увеличивает время ответа пользователю на приём обращения.
